...
Ss::SettingScreen::SettingScreen(QWidget *parent) : QWidget(parent), ui(new Ui::SettingScreen)
{
    ui->setupUi(this);
...
Ss::SettingItem::SettingItem(QWidget *parent) : QWidget(parent), contentVisible(false)
{
    setStyleSheet(
        "QWidget {"
...
void Ss::SettingScreen::showEvent(QShowEvent *event)
{
...
// 查找子控件，没找到返回 nullptr
QPushButton *Ss::SettingScreen::getButton(const QString &name) const
{
    return findChild<QPushButton *>(name);
}

Ss::SettingItem::~SettingItem()
{
    // 不需要手动删除控件，Qt 的对象树机制会自动管理
}

// 设置内容
void Ss::SettingItem::setContent(const QString &content)
{
...
// 设置内容是否可见
void Ss::SettingItem::setContentVisible(bool visible)
{
...
// 获取内容是否可见
bool Ss::SettingItem::isContentVisible() const
{
    return contentVisible;
}

// 更新标签内容
void Ss::SettingItem::updateContentDisplay()
{
    if(contentVisible)
    {
