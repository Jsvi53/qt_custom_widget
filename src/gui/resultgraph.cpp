...
void ResultGraph::graphInit()
{
    ...
    loadCSVData("D:/shawei/temp/custom_widget/database/12k_144.csv", baData, deData, feData);
    ...
    QVector<double> tempdeData;
    tempdeData = deData.mid(0, displayPoints);
    // 启动异步FFT计算
    QApplication::setOverrideCursor(Qt::WaitCursor);
    QFuture<void> future = QtConcurrent::run([this, tempdeData]() {
        QVector<double> freq, amp;
        computeFFT(tempdeData, freq, amp);
...
ResultGraph2::ResultGraph2(QWidget *parent) : QWidget(parent), ui(new Ui::ResultGraphScreen)
{
    ui->setupUi(this);
    ...
    // 连接信号槽
    connect(&fftWatcher, &QFutureWatcher<void>::finished, this, [this]() { QApplication::restoreOverrideCursor(); });
    connect(this, &ResultGraph2::fftCompleted, this, &ResultGraph2::handleFftResults);

    graphInit();
}

ResultGraph2::~ResultGraph2()
{
    fftWatcher.cancel();
    fftWatcher.waitForFinished();
    delete ui;
...
void ResultGraph2::graphInit()
{
    ...
    const int       displayPoints = 1000;  // 显示部分时域数据（前100点）
    QVector<double> time(displayPoints);
    const double    Fs = 12000.0;
    for(int i = 0; i < displayPoints; ++i)
    {
    ...
    QVBoxLayout *uplayout   = new QVBoxLayout(ui->upGraph);
    QVBoxLayout *downlayout = new QVBoxLayout(ui->downGraph);

    // 时域图初始化
    resultTimeGraph = new QCustomPlot();
    downlayout->addWidget(resultTimeGraph);
    downlayout->setContentsMargins(10, 0, 0, 10);
    resultTimeGraph->setBackground(Qt::transparent);
    resultTimeGraph->replot();
    resultTimeGraph->xAxis->setLabel("t[s]");
    resultTimeGraph->yAxis->setLabel("v[mm/s]");
    resultTimeGraph->addGraph();
    resultTimeGraph->graph(0)->setPen(QPen(Qt::darkBlue));
    resultTimeGraph->addGraph();
    resultTimeGraph->graph(0)->setData(time, baData.mid(0, displayPoints));
    resultTimeGraph->replot();
    // 自适应调整坐标轴范围
    resultTimeGraph->rescaleAxes(true);

    // 频域图初始化
    resultFreqGraph = new QCustomPlot();
    uplayout->addWidget(resultFreqGraph);
    uplayout->setContentsMargins(10, 0, 0, 10);
    resultFreqGraph->setBackground(Qt::transparent);
    resultFreqGraph->replot();
    resultFreqGraph->xAxis->setLabel("f[Hz]");
    resultFreqGraph->yAxis->setLabel("Amplitude[mm/s]");
    resultFreqGraph->addGraph();
    resultFreqGraph->graph(0)->setPen(QPen(Qt::red));

    QVector<double> tempdeData;
    tempdeData = deData.mid(0, displayPoints);

    QApplication::setOverrideCursor(Qt::WaitCursor);        // 启动异步FFT计算
    QFuture<void> future = QtConcurrent::run([this, tempdeData]() {
        QVector<double> freq, amp;
        computeFFT(tempdeData, freq, amp);
        emit fftCompleted(freq, amp);
...
void ResultGraph2::addOverallValue(overallValuesPrograssBar *overallBar, int value, QGridLayout *gridLayout)
{
    overallBar->setRange(0, 100);
    overallBar->setValue(value);
    gridLayout->addWidget(overallBar, 1, 0);  // 添加进度条到布局, 位置在第二行第一列
...
void ResultGraph2::loadCSVData(const QString &filePath, QVector<double> &baData, QVector<double> &deData, QVector<double> &feData)
{
    QFile file(filePath);
    if(!file.open(QIODevice::ReadOnly | QIODevice::Text))
    {
...
void ResultGraph2::handleFftResults(QVector<double> freq, QVector<double> amp)
{
    resultFreqGraph->graph(0)->setData(freq, amp);
    resultFreqGraph->rescaleAxes(true);
    resultFreqGraph->replot();
    QApplication::restoreOverrideCursor();
}

void ResultGraph2::computeFFT(const QVector<double> &baData, QVector<double> &freq, QVector<double> &amp)
{
    const int fftSize = baData.size();
    if(fftSize < 1)
        return;
